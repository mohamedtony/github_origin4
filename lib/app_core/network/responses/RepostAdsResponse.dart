...
part 'RepostAdsResponse.g.dart';

@JsonSerializable()
class RepostAdsResponse {
  int? status;
  String? message;
  Data? data;
  String? pagination;

  RepostAdsResponse({this.status, this.message, this.data, this.pagination});

  RepostAdsResponse.fromJson(Map<String, dynamic> json) {
    status = json['status'];
...
class User {
  ...
  User({this.id, this.username, this.country, this.area, this.phone});
...
class Attachments {
  ...
  Attachments(
      {this.id, this.sort, this.adsId, this.type, this.path, this.videoId});
