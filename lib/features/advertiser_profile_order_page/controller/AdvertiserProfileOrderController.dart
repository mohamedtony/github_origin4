...
//=========================================================================================
class AdvertiserProfileOrderController extends GetxController with GetTickerProviderStateMixin{
  var tabIndex = 0.obs;
  var isOpend = false;
  var position = -1;
  var imagePath = ''.obs;
...
/*    if(isOpend){
  ...
  var isShowChannelsClicked = false.obs;
  var isShowDetailsClicked = false.obs;
...
//================================== dicount sheet ==========================================
  ...
  Future<void> onSendRequestClicked(BuildContext context) async {
    if (categoryId == -1) {
      ...
      return;
    } else if (isFlixble.isTrue &&
        toDate.value.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(
...
/*    myDio.MultipartFile? imageCopon;
    ...
    if(imageFideoFiles!=null && imageFideoFiles!.isEmpty){
      print("myFilesEmpty");
    }else{
      print("videosNum"+imageFideoFiles!.length.toString());
    }
/*    requestAdvertiseController.attatechedFilesImageAndVideo.forEach((element) async {
    ...
    Map<String, dynamic> mymap = {
      ...
      int.parse(selectedCounterController.text):1,
      ...
      "location[lat]": locationModel.lat,
      "location[lng]": locationModel.lng,
