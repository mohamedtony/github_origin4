...
class _DragTargetState<T> extends State<DragTarget<T>> {
  ...
  final List<_DragAvatar<T>> _rejectedAvatars = <_DragAvatar<T>>[];

  bool didEnter(_DragAvatar<T> avatar) {
  ...
  Widget build(BuildContext context) {
    ...
    return MetaData(
      ...
      child: widget.builder(context, _mapAvatarsToData<T>(_candidateAvatars), _mapAvatarsToData<T>(_rejectedAvatars)),
    );
  }
}
